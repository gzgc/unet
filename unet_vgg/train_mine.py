...
def train_net(net, device, data_path, epochs=200, batch_size=10, lr=0.0001):
    ...
    for epoch in range(epochs):
        ...
        for image, label in train_loader:
            ...
            if loss < best_loss:
                best_loss = loss
                torch.save(net.state_dict(), 'unet_vgg_8k_78.pth')
            loss.backward()
            optimizer.step()
        print(f'epoch:{epoch}-----> loss_mean:{loss_sum/nnnnn}')
        torch.save(net.state_dict(), f'checkpoint/unet_vgg_epoch_{93+epoch}.pth')
    print(time.time())
    print('end.....')
...
if __name__ == '__main__':
    ...
    net.load_state_dict(torch.load('checkpoint/unet_vgg_epoch_93.pth'))
